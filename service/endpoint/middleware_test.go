...
func TestMakeCorrelationIDMiddleware(t *testing.T) {
	...
	var genID = rand.Uint64()
	var genIDStr = strconv.FormatUint(genID, 10)
	mockFlaki.id = genID
	endpoints.NextID(context.Background())
	assert.True(t, mockLogger.Called)
	assert.Equal(t, genIDStr, mockLogger.CorrelationID)
	...
	// Set the id generated by Flaki.
	genID = rand.Uint64()
	genIDStr = strconv.FormatUint(genID, 10)
	mockFlaki.id = genID
